...
const std::string XMLElement::ToString() const
{
    ...
    line += "<" + name;
    for( XMLAttribute* a = this->_rootAttribute; a; a = a->_next ) {
        line += " " + a->ToString();
    }
    if(this->NoChildren()) {
        if(this->GetText())
            line += ">" + std::string(this->GetText()) + "</" + name + ">";
        else {
            line += "/>";
        }
    } else {
        line += ">";
        if(this->GetText())
            line += std::string(this->GetText());
        for (const XMLElement* child = this->FirstChildElement(); child; child = child->NextSiblingElement()){
            std::string temp = child->ToString();
            line += temp;
        }
        line += "</" + name + ">";
    }
